...
abstract class _ClientChatMixin implements _ClientWrapper {
  Future<Message> sendMessage(Message message) {
    ...
    http
        .post(Uri.https('${_getUrl()}', '/chat.sendMessage'),
            headers: {
              'X-User-Id': _auth._id,
  ...
  Future<void> reactMessage(String messageId, String emoji, [bool shouldReact]) {
    ...
    http.post(Uri.https('${_getUrl()}', '/chat.react'), headers: headers, body: body).then((response) => completer.complete(null)).catchError((error) => completer.completeError(error));
    return completer.future;
  }
}
